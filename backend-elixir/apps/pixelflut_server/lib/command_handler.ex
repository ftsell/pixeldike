defmodule PixelflutServer.CommandHandler do
  ...
  ## Examples
  Sending an invalid input returns the error "invalid_command"
      iex> PixelflutServer.CommandHandler.parse_and_handle("hello my dear server")
      "invalid_command"

  Setting a pixel returns nothing
      iex> PixelflutServer.CommandHandler.parse_and_handle("PX 0 0 FFFFFF")
      :empty
  ...
  defp parse(line) do
    case String.split(String.downcase(line)) do
      ["px", x, y, string_color] -> case parse_color(string_color) do
                                      {:ok, color} -> {:ok, {:set_pixel, String.to_integer(x), String.to_integer(y), color}}
  ...
  defp handle({:get_state, _algorithm}) do
    {:error, :not_implemented}
  end

  defp parse_color(color) do
    Base.decode16(color, [case: :lower])
  end
end
