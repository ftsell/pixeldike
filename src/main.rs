...
fn main() {
    ...
    let mut map: Vec<Vec<String>> = vec![vec![String::from(BACKGROUND_COLOR); Y_SIZE]; X_SIZE];
        ...
        if msg.is_err() {
            ...
            if cmd.is_err() {
                ...
                let answer = match cmd { 
                    ...
                    Command::PX(x, y, color) => cmd_px(&mut map, x, y, color)
                };
...
fn cmd_px(map: &mut Vec<Vec<String>>, x: usize, y: usize, color: String) -> String {
    ...
    let column: &mut Vec<String> = map.get_mut(x).unwrap();
