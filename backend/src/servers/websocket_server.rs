...
impl WsServer {
    ...
    fn handle_client(self, socket: Framed<TcpStream, MessageCodec<OwnedMessage>>) {
        ...
        let answer_fut = strm
            ...
            .and_then(|(_m, sink)| sink.send(OwnedMessage::Close(None)).then(|_| Ok(())));
...
impl PxServer for WsServer {
    fn start(self, port: u16) {
        ...
        let server_fut = listener
            ...
            .and_then(|(upgrade, addr)| {
                upgrade
                    .use_protocol(SUB_PROTO)
                    .accept()
                    .map_err(|e| eprintln!("[WS] Error performing protocol upgrade: {}", e))
                    .map(move |(socket, _headers)| (socket, addr))
            })
            // Handle PX command
            .for_each(move |(socket, _addr)| {
                // TODO Write address based limitations (number of clients from same address
    ...
    fn cmd_get_state(
        ...
        return self.map.get_state(x, y).map(Some);
    }
}
